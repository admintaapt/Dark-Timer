...
$(document).ready(function () {
    ...
    fetch(`https://api.pexels.com/v1/search?query=${encodeURIComponent(businessModel)}?page=3&per_page=4`, {
        headers: {
            Authorization: apiKey
        }
    })
        .then(response => response.json())
        .then(data => {
            const thumbnails = ['pexels-thumb1', 'pexels-thumb2', 'pexels-thumb3', 'pexels-thumb4'];
            ...
            data.photos.forEach((photo, index) => {
                ...
                thumbElement.src = photo.src.medium;  // Miniatura pequeña
    ...
    fetch(`https://api.pexels.com/v1/search?query=${encodeURIComponent(businessModel)}&per_page=1`, {
        ...
        .then(data => {
            if (data.photos.length > 0) {
                const splitSectionImage = document.getElementById('split-section-image');
                if (splitSectionImage) {
                    // Establecer la imagen obtenida en el elemento con ID 'split-section-image'
                    splitSectionImage.src = data.photos[0].src.large;
                }
            }
        })
        .catch(error => console.error('Error fetching split section image from Pexels:', error));
        const imageIds = ['pexels-thumb5', 'pexels-thumb6', 'pexels-thumb7'];

        // Realiza una solicitud para obtener imágenes desde Pexels
        fetch(`https://api.pexels.com/v1/search?query=${encodeURIComponent(businessModel)}?page=2&per_page=3`, {
          headers: {
            Authorization: apiKey
          }
        })
        .then(response => response.json())
        .then(data => {
          // Asegúrate de que hay suficientes imágenes en la respuesta
          if (data.photos && data.photos.length >= imageIds.length) {
            data.photos.forEach((photo, index) => {
              // Obtiene el ID de la imagen correspondiente en el HTML
              const imageElement = document.getElementById(imageIds[index]);
              if (imageElement) {
                // Establece la imagen en el atributo src y el alt
                imageElement.src = photo.src.large;
                imageElement.alt = photo.alt;
              }
            });
          } else {
            console.error('No se encontraron suficientes imágenes en la respuesta de Pexels.');
          }
        })
        .catch(error => {
          console.error('Error al cargar las imágenes desde Pexels:', error);
        });
...
if (landingData) {
    ...
    if (landingData['button-1-link']) {
        ...
        if (button1) {
            button1.href = landingData['button-1-link'];
        }
    }
